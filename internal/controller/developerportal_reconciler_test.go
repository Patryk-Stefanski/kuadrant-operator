...
func TestDeveloperPortalReconciler(t *testing.T) {
	...
	t.Run("Subscription", func(subT *testing.T) {
		...
		assert.Assert(subT, is.Len(events, 11))
		// Kuadrant resource
		...
		// Role
		assert.DeepEqual(subT, events[2].Kind, ptr.To(RoleGroupKind))
		...
		// RoleBinding events
		assert.DeepEqual(subT, events[5].Kind, ptr.To(RoleBindingGroupKind))
		assert.DeepEqual(subT, events[5].EventType, ptr.To(controller.DeleteEvent))
		assert.DeepEqual(subT, events[6].Kind, ptr.To(RoleBindingGroupKind))
		assert.DeepEqual(subT, events[6].EventType, ptr.To(controller.UpdateEvent))
		// ServiceAccount events
		assert.DeepEqual(subT, events[7].Kind, ptr.To(ServiceAccountGroupKind))
		assert.DeepEqual(subT, events[7].EventType, ptr.To(controller.DeleteEvent))
		assert.DeepEqual(subT, events[8].Kind, ptr.To(ServiceAccountGroupKind))
		assert.DeepEqual(subT, events[8].EventType, ptr.To(controller.UpdateEvent))
		// Deployment events
		assert.DeepEqual(subT, events[9].Kind, &kuadrantv1beta1.DeploymentGroupKind)
		assert.DeepEqual(subT, events[9].EventType, ptr.To(controller.DeleteEvent))
		assert.DeepEqual(subT, events[10].Kind, &kuadrantv1beta1.DeploymentGroupKind)
		assert.DeepEqual(subT, events[10].EventType, ptr.To(controller.UpdateEvent))
	})
	...
	t.Run("Skip reconciliation when ClusterRole not in topology", func(subT *testing.T) {
		topology := buildTopologyWithKuadrant(subT, true)
		// Reconcile should succeed but not create resources due to missing ClusterRole in topology
		...
		assert.Assert(subT, apierrors.IsNotFound(err), "ServiceAccount should not be created when ClusterRole is missing from topology")
	})

	// Note: Full resource creation tests (ServiceAccount, RoleBindings, Deployment) are covered in integration tests
